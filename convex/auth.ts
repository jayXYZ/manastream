...
async function initializeNewUser(ctx: MutationCtx, userId: Id<"users">) {
  ...
  await ctx.runMutation(internal._overlays.match.internalCreateMatchOverlay, {
    tournamentId,
    name: "Match Overlay 1",
  });
  await ctx.runMutation(internal._overlays.match.internalCreateMatchOverlay, {
    tournamentId,
    name: "Match Overlay 2",
  });
  await ctx.runMutation(internal._overlays.match.internalCreateMatchOverlay, {
    tournamentId,
    name: "Match Overlay 3",
  });

  // Create a card overlay
  await ctx.runMutation(internal._overlays.card.internalCreateCardOverlay, {
    tournamentId,
    name: "Card Overlay",
  });
  ...
  await ctx.runMutation(
    internal._overlays.commentary.internalCreateCommentaryOverlay,
    {
      tournamentId,
