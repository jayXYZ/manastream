  ...
  handler: async (ctx) => {
    ...
    if (newMode === "auto") {
      await ctx.scheduler.runAfter(
        0,
        internal._spicerack.pollForTournamentData,
        {
...
export const pollForTournamentData = internalAction({
  ...
  handler: async (ctx, args) => {
    const { tournament, settings, featureMatches } = await ctx.runQuery(
      internal._spicerack.getDataForPolling,
      { tournamentId: args.tournamentId },
    );
    if (tournament.mode === "manual") {
      return;
    }
    if (!tournament.spicerackTournamentId || !settings.spicerackApiKey) {
    ...
    const currentRoundFeatureMatches = await ctx.db
      ...
      .withIndex("by_tournament_and_round", (q) =>
        q
          ...
          .eq("roundNumber", tournament.spicerackCurrentRoundNumber ?? 0),
      )
      .collect();
